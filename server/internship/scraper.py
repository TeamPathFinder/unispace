            ...
            try:
                ...
                for job in jobs:
                    title = job.find_element(
                        by=By.CLASS_NAME, value="BjJfJf.PUpOsf"
                    ).text
                    ...
                    # SET POSTED DATE TO ACTUAL DATE
                    # 'today' will be set to today's date
                    # '1 day ago' will be set to yesterday's date, and so on.
                    # If the date is unknown, set it to 2 days ago by default.
                    current_date = datetime.now()
                    if "days" in posted_date or "day" in posted_date:
                        posted_date = int(posted_date.split(" ")[0])
                        actual_date_posted = current_date - timedelta(days=posted_date)
                    elif "hours" in posted_date:
                        posted_date = 0
                        actual_date_posted = current_date
                    else:
                        # If the date is unknown, set to 2 days ago
                        # So it does not appear as "today"
                        posted_date = 2
                        actual_date_posted = current_date - timedelta(days=2)
                ...
                # Skip to the next city/country
                print(f"Timeout occurred while loading page for {city}, {country}: {e}")
        print("=====================================")
